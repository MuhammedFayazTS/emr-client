...
import { createDepartmentSchema, type CreateDepartmentInput } from "../validation/department.schema";
...
export default function DepartmentForm() {
    ...
    const { mutate: createDepartment, isPending: isCreatePending, error: createError } = useCreateDepartment();
    const { mutate: updateDepartment, isPending: isUpdatePending, error: updateError } = useUpdateDepartment();
    ...
    const onSubmit = (values: CreateDepartmentInput) => {
        if (isEdit) {
            updateDepartment({ id, payload: values }, {
    ...
    return (
        <form onSubmit={form.handleSubmit(onSubmit)} noValidate>
            <FieldGroup>
                ...
                {
                    isDepartmentLoading ? <Skeleton className="h-24" />
                        :
                        (<>
                            <DefaultTextInput
