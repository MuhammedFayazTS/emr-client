...
export const departmentApi = {
  getList: async (filters: DepartmentFilters): Promise<PaginatedResponse<Department[]>> => {
    const res = await axiosInstance.get<PaginatedResponse<Department[]>>("/departments", {
      params: filters,
    });
    return res.data;
  },
  ...
  getOne: async (id: string): Promise<Department> => {
    const res = await axiosInstance.get<ApiSuccessResponse<Department>>(`/departments/${id}`);
    return res.data.data;
  },

  create: async (payload: CreateDepartmentPayload): Promise<Department> => {
    const res = await axiosInstance.post<ApiSuccessResponse<Department>>("/departments", payload);
    return res.data.data;
  },
