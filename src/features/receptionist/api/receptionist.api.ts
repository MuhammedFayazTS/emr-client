...
export const receptionistApi = {
  getList: async (filters: ReceptionistFilters): Promise<PaginatedResponse<Receptionist[]>> => {
    const res = await axiosInstance.get<PaginatedResponse<Receptionist[]>>("/receptionists", {
      params: filters,
    });
    return res.data;
  },
  ...
  getOne: async (id: string): Promise<Receptionist> => {
    const res = await axiosInstance.get<ApiSuccessResponse<Receptionist>>(`/receptionists/${id}`);
