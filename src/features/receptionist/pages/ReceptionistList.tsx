...
export default function ReceptionistList() {
  ...
  // ── Queries / Mutations ─────────────────────────────────
  const { data, isLoading, isError } = useReceptionists({
    cursor: currentCursor,
    limit: PAGE_SIZE,
    search: debouncedSearch,
  });

  const { mutate: deleteReceptionist, isPending: isDeleting } = useDeleteReceptionist();
  ...
  const viewDetails = useMemo(() => {
    ...
    return [
      ...
      {
        label: "Created At",
        value: new Date(selectedReceptionist.createdAt).toLocaleDateString("en-US", {
          year: "numeric",
          month: "short",
          day: "numeric",
        }),
