...
import { createDoctorScheduleSchema, type CreateDoctorScheduleFormInput, type CreateDoctorScheduleOutput } from "../validation/doctorSchedule.schema";
import { useCreateDoctorSchedule, useUpdateDoctorSchedule } from "../hooks/useDoctorScheduleMutations";
...
export default function DoctorScheduleForm() {
    ...
    const { mutate: createSchedule, isPending: isCreatePending, error: createError } = useCreateDoctorSchedule();
    const { mutate: updateSchedule, isPending: isUpdatePending, error: updateError } = useUpdateDoctorSchedule();
    ...
    const onSubmit = (payload: CreateDoctorScheduleOutput) => {
        if (isEdit) {
            updateSchedule({ id: id as string, payload }, {
    ...
    useEffect(() => {
        if (isEdit && scheduleData) {
            ...
            const mergedWeek = emptyWeek.map((defaultDay) => {
                const match = scheduleData.workingDays?.find(
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} noValidate>
            <FieldGroup>
                ...
                <div className="flex justify-end gap-2 pt-2">
                    <Button type="button" variant="outline" onClick={() => navigate("/doctor-schedules")} disabled={isPending}>
                        Cancel
                    </Button>
                    <Button type="submit" disabled={isPending || isLoading}>
                        {isEdit ? (isPending ? "Updating..." : "Update Schedule") : (isPending ? "Creating..." : "Create Schedule")}
                    </Button>
                </div>
            </FieldGroup>
        </form>
