...
export const appointmentApi = {
  getList: async (filters: AppointmentFilters): Promise<PaginatedResponse<Appointment[]>> => {
    const res = await axiosInstance.get<PaginatedResponse<Appointment[]>>("/appointments", {
      params: filters,
    });
    return res.data;
  },
  ...
  getOne: async (id: string): Promise<Appointment> => {
    const res = await axiosInstance.get<ApiSuccessResponse<Appointment>>(`/appointments/${id}`);
    return res.data.data;
  },

  create: async (payload: CreateAppointmentPayload): Promise<Appointment> => {
    const res = await axiosInstance.post<ApiSuccessResponse<Appointment>>("/appointments", payload);
    return res.data.data;
  },
