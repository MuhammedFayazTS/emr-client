...
export default function AppointmentForm() {
    ...
    const { mutate: createAppointment, isPending: isCreatePending, error: createError } = useCreateAppointment();
    const { mutate: updateAppointment, isPending: isUpdatePending, error: updateError } = useUpdateAppointment();
    const { data: appointmentData, isLoading: isAppointmentLoading } = useGetAppointment(id as string);

    const { data: patientsData, isLoading: isPatientsLoading } = usePatients({ limit: 100, isActive: true });
    ...
    const doctorOptions = useMemo(
        () => formatOptions(doctorsData?.data || []),
        [doctorsData]
    );
                    ...
                    {isLoading ? (
                        ...
                        <>
                            <div className="rounded-lg border p-4 space-y-2 text-sm">
                                <p><span className="text-muted-foreground">Appointment #:</span> {appointmentData?.appointmentNumber}</p>
                                <p><span className="text-muted-foreground">Patient:</span> {resolveEntityLabel(appointmentData?.patientId ?? "—")}</p>
                                <p><span className="text-muted-foreground">Doctor:</span> {resolveEntityLabel(appointmentData?.doctorId ?? "—")}</p>
                                <p><span className="text-muted-foreground">Department:</span> {resolveEntityLabel(appointmentData?.departmentId ?? "—")}</p>
                                <p><span className="text-muted-foreground">Date:</span> {appointmentData?.date ? formatAppointmentDate(appointmentData.date) : "—"}</p>
                                <p><span className="text-muted-foreground">Time:</span> {appointmentData ? `${appointmentData.startTime} – ${appointmentData.endTime}` : "—"}</p>
                                <p><span className="text-muted-foreground">Status:</span> {appointmentData?.status ?? "—"}</p>
                            </div>
