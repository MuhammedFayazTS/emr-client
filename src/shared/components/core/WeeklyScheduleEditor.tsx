  ...
  return (
    <div className="space-y-3">
      {fields.map((field, dayIndex) => (
        <DayRow key={field.id} control={control} name={name} dayIndex={dayIndex} />
      ))}
    </div>
  );
}
...
function DayRow<TFormValues extends FieldValues>({
  ...
  const dayLabel = useWatch({
    control,
    name: `${dayPath}.dayOfWeek` as Path<TFormValues>,
  }) as string;
  ...
  return (
    <div className={cn("rounded-lg border p-3 transition-opacity", !isWorking && "opacity-60")}>
        ...
        <div className="mt-3 space-y-2">
          {sessionFields.map((session, sessionIndex) => (
            <div key={session.id} className="flex items-center gap-2">
              ...
              <TimeField
                control={control}
                name={`${sessionsPath}.${sessionIndex}.startTime` as Path<TFormValues>}
              />
              <span className="text-muted-foreground text-sm">to</span>
              <TimeField
                control={control}
                name={`${sessionsPath}.${sessionIndex}.endTime` as Path<TFormValues>}
              />
              <Button
                type="button"
                variant="ghost"
          ...
          <Button
            ...
            size="sm"
            onClick={() =>
              append({ name: "", startTime: "09:00", endTime: "17:00" } as unknown as FieldArray<
                TFormValues,
...
function SwitchField<TFormValues extends FieldValues>({
  control,
  name,
}: {
  control: Control<TFormValues>;
...
function NameField<TFormValues extends FieldValues>({
  control,
  name,
}: {
  control: Control<TFormValues>;
  ...
  return (
    <div>
      <Input type="text" placeholder="Session name" className="w-36" {...field} />
      {error && <p className="text-xs text-destructive mt-1">{error.message}</p>}
    </div>
  );
}

function TimeField<TFormValues extends FieldValues>({
  control,
  name,
}: {
  control: Control<TFormValues>;
